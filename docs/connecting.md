...
The mint call is `POST /v1/client-tokens` with `Authorization: Bearer <credential>` and a body of `{ name, allowedOrigins: [origin], agentIds, environment }`. The token is re-minted when the site origin or the chosen agent changes.
...
* **Scopes:** server-fixed `CLIENT_TOKENS:WRITE` + `AGENTS:READ`. The plugin sends `scope=` empty; it never requests or expects `*`.
